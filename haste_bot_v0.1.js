...
const version = "Bot version: 0.1 ***Alpha***";
...
{
	const pre = message.guild.emojis.get("349604451281141762").toString().toLowerCase();
    ...
    //Basic text commands
	if(text.startsWith(pre+" ping")) message.channel.send("Pong! `"+(Date.now()-message.createdTimestamp)+" ms`");

    else if(text.startsWith(pre+" price"))
    {
        embed.setTitle("Here's Haste's current prices!");
        embed.setAuthor(config.username, client.user.avatarURL);
...
logs.send(banEmbed);
}, (reason) => {console.log("Ban log rejected because:\n"+reason);})
})
...
logs.send(kickEmbed);
}, (reason) => {console.log("Kick log rejected because:\n"+reason);})
})
